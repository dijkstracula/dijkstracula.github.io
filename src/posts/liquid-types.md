...
However, as a form of abstract interpretation[^6], Milner's _bon
...
hierarchy for `List`,[^7], indexed
types[^8], or generalized algebraic
datatypes[^9], we need to place constraints on terms of type
...
inherent runtime semantics (for a more complete example, see[^10]).
...
on line 7 of Figure 1 is far shorter and more descriptive than its
equivalent on line 24 of Figure 3.
...
A _dependent_ type system[^11], however, is one in which types and terms
...
checking like SLAM[^12], BLAST[^13] or Houdini[^14] at it"
...
it's been[^15], limiting the richness of the space of counterproofs
...
propositions in constructive logic[^2].  In that way, the reading of this
...
full automation in their models[^16], or model checker implementers are
required to add new axioms on a per-data structure
basis[^17] or hope that their system's
heuristics[^18] will not cause verification to spiral out of
control.

## Unifying the two approaches with logically-qualified types

A _refinement type_[^19] is the pairing of an ordinary,
...
In addition to safety improvements, prior work[^11] showed the clear
...
(LiQuid) type_[^20].  Ideally, we would be able to pick and
...
::: margin-note
When we say "the best" annotation, that is to say: trivially typing everybody
to ⊤ won't do!
:::
The procedure described in the authors' initial work[^20] is
...
expressions as opposed to the type _checking_ problem, where a program
...
later in their refinement work for C[^22], and liquid
records and algebraic datatypes would appear later
still[^23].

Base types are inferred via the Hindley-Milner (H-M) type inference
algorithm[^2], an absolute bog-standard procedure that we describe here only
...
incrementally[^2] or interleaved with each other, which is important for
...
into the realm of undecidability[^24].  But, the tradeoff
...
liquid types paper[^20] mentions using an SMT solver almost as
...
non-trivial and unanticipated ways[^2].

Subtyping is defined in terms of a _subsumption_ relation between two types:
recalling Liskov[^27], `T1` is a subtype of `T2` if occurrences of
...
model-checking solutions([^12], [^13], [^14]), a liquid type
...
variables is extremely difficult[^18], and is a subject of
extensive and active research.

Liquid types eschew the need for quantifiers by factoring reasoning
about them into the type system[^28]. Quantified
...
turns out to be a very useful heuristic[^29]. It's worth
...
ML[^11] and previous ML refinement type work[^19].
Generalizing followup work came soon after, focusing on imperative programs in
C[^22] and lazy functional
languages[^23].  More recently, follow-up work has been
done in integrating liquid types into a gradual typing
environment[^26] and industrial languages like
TypeScript[^30].

More broadly, integration of SMT solvers into programming languages has been
explored by both higher-order function languages[^31] and imperative, OO-style
ones[^16].
...
[^3]: Mimram, Samuel. "Program = Proof" (2021).

[^4]: Milner, Robin. "A theory of type polymorphism in programming" (1978). Journal of Computer and System Sciences.

[^5]: Vazou, Niki, et al. "LiquidHaskell: Experience With Refinement Types in the Real World". Programming Languages Design and Implementation (PLDI) 2014.

[^6]: Cousot, Patrick and Cousot, Radhia. "Abstract Interpretation: A Unified Lattice Model for Static Analysis of Programs by Construction or Approximation of Fixpoints" (1977). POPL.

[^7]: ScalaCats NonEmptyList documentation.

[^8]: Zenger, Christoph. "Indexed Types" (1997). Theoretical Computer Science.

[^9]: Xi, Hongwei and Pfenning, Frank. "Dependent Types in Practical Programming" (1999). POPL.

[^10]: "Shapeless: Generic programming for Scala". <https://github.com/milessabin/shapeless>

[^11]: Xi, Hongwei and Pfenning, Frank. "Eliminating Array Bound Checking Through Dependent Types" (1998). PLDI.

[^12]: Ball, Thomas and Rajamani, Sriram. "The SLAM Project: Debugging System Software via Static Analysis" (2002). POPL.

[^13]: Beyer, Dirk, et al. "Lazy Abstraction" (2002). POPL. Berkeley Lazy Abstraction Software Verification Tool (BLAST).

[^14]: Flanagan, Cormac and Qadeer, Shaz. "Predicate Abstraction for Software Verification" (2002). POPL.

[^15]: McMillan, Kenneth L. "Lazy Annotation for Program Testing and Verification" (2010). CAV. ["The Road to Nowhere" approach]

[^16]: Leino, K. Rustan M. "Dafny: An Automatic Program Verifier for Functional Correctness" (2010). LPAR.

[^17]: Ranise, Silvio and Zarba, Calogero G. "A Theory of Singly-Linked Lists and Its Extensible Decision Procedure" (2006). SEFM.

[^18]: Leino, K. Rustan M. and Pit-Claudel, Clément. "Trigger Selection Strategies to Stabilize Program Verifiers" (2016). CAV.

[^19]: Freeman, Tim and Pfenning, Frank. "Refinement Types for ML" (1991). PLDI.

[^20]: Rondon, Patrick, et al. "Liquid Types" (2008). PLDI.

[^22]: Rondon, Patrick, et al. "Low-Level Liquid Types" (2010). POPL.

[^23]: Vazou, Niki, et al. "Refinement Types for Haskell" (2014). ICFP.

[^24]: Wells, J. B. "Typability and type checking in System F are equivalent and undecidable" (1999). Annals of Pure and Applied Logic.

[^25]: Kawaguchi, Ming, et al. "Type-based Data Structure Verification" (2009). PLDI.

[^26]: Lehmann, Nico and Tanter, Eric. "Gradual Liquid Type Inference" (2018). OOPSLA.

[^27]: Liskov, Barbara. "Data Abstraction and Hierarchy" (1987). OOPSLA Keynote.

[^28]: Jhala, Ranjit and Vazou, Niki. "Refinement Types: A Tutorial" (2020). Foundations and Trends in Programming Languages.

[^29]: Vazou, Niki. "Liquid Haskell: Haskell as a Theorem Prover" (2016). PhD Thesis, UC San Diego.

[^30]: Feldthaus, Asger and Møller, Anders. "Adding Refinement Types to TypeScript" (2020). ECOOP.

[^31]: Swamy, Nikhil, et al. "F*: A Higher-Order Effectful Language Designed for Program Verification" (2016). POPL.
