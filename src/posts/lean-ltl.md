---
layout: post.njk
title: "Proofs over Reactive Programs in Lean 4"
date: 2026-03-05
...
essentially stateless, and certainly has no notion of concurrent parts
operating in tandem.
...
awhile, and ultimately produces some output.  We usually just call
...
responding to calls from a model, are reactive.  The Harel and Pnueli paper
has a delightful figure contrasting the two: transformational programs are
the usual black box with input arrows going in one side and output arrows going
out the other, whereas reactive programs are "black cacti", with input and
output arrows spiking all around the box.

::: margin-note
More technically, terms that appear in types must be total in order for the
type system's logic to be consistent.  To see why, remember that contradictions
derive from trying to prove `False`, which is a type with no values that can be
constructed.  But, what could the return type of the function `def loop
x = loop x`?  Well, it never produces a value at all, so any return
type is just as good as any other; so, it's not incorrect to say that
`loop` returns a value of type `False`!  And, since we've seen, `False`
proves anything, non-terminating computation punches a hole in our logic.
...
:::
Proving properties about reactive programs is going to be a lot different tnan
what we've done before, though.  First of all, the Lean functions we've written
up to now have to provably terminate in order to satisfy the typechecker --
that's an inherit requirement for dependent types, since types and program
expressions intermingle -- so we're going to have to write our programs
differently.
...
examples of a reactive system in Lean, a vending machine that sells cans of
pop.  A pop machine has a bunch of state: it's got some number of cans of pop
(let's say two, to keep things straightforward), some number of coins that have
been inserted, and zero or one cans of some pop flavour in the dispenser.
...
ToString`. (The other typeclass we are making `Flavour` an instance of,
`DecidableEq`, we'll get to soon enough.)

Suppose any given vending machine starts fully stocked:
...
I can think of a few external events that can act on a vending machine.  A
customer can drop a coin in the slot, press one of the "select flavour" button,
and can take a can out of the dispenser.  I suppose the machine can be
restocked, too.
...
### Acting on a State with the pop machine's operational semantics
...
the pop machine's operation.  The ten dollar computer science term for this is
...
a pop machine with any (nonnegative) number of cans of each flavour, but,
...
big hint: it relates to that notion of reachability and valid states...
:::
These three pieces - our state, action, and step definitions - define the
_abstract machine_ for the pop machine.  All we need now is a way to write down
and execute programs on this abstract machine and we'll have completed our
...
otherwise pure functional program.  Let's do the same thing here!
...
be `VMState`, and alpha can vary depending on what output we want a given
...
the final state of the pop machine: as expected, it has a single coin in it.
...
We can now complete the implementation of `getOrange`, which encodes the effectful operations of dropping in
two coins, choosing a flavour, and taking it out of the machine.  The final state is the same as before but now
our output value is the expected flavour.  Excellent!
...
about `s`?  Your pop machine might differ from mine, but as a first pass,
...
specifications, and indeed, we'll see there are some problems with the one
I've shown you.  
...
### Proofs of valid `Choose`s necessitate reasoning about the Nats
...
### Proofs of `TakeItem` are just an equality check
